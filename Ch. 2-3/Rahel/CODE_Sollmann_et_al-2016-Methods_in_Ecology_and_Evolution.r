...
VAR<-as.matrix(cbind(sbdata$covs$DIST, 
                     sbdata$covs$TEMP, sbdata$covs$PREY5))	# extract abundance covariates
...
data.in<-list(spec=nsp, nG=nG, pi=pix, v=v, db=db, nsites=nSeg,nVAR=nVAR,
              y=y, nind=nind , VAR = VAR, offset=seglength,
              dclass=dclass, species=species, site=site, OBSVAR=OBSVAR,
              flock=flock, Nmax=Nmax)

inits<-function(){list(N=Nin, mu_a=runif(1,0,1), tau_a=runif(1,0,1), mu_b=runif(nVAR,0,1), tau_b=runif(nVAR,0,1),
                       asig=runif(nsp,5,6), mu_s = runif(1, 5, 6), sig_s=runif(1) )}  

params<-c('asig', paste('bsig[', 2:4,']', sep=''), 'mu_s', 'sig_s', 'mu_a', 'sig_a', 'mu_b', 'sig_b',
          'Nspec', 'Atot', 'mu', 'r', 'T1', 'T1new', 'Tobs', 'Tobsnew', 'Tg1', 'Tg1new', 'r.N', 'alpha', 
          'beta')
...
mod<-jags.model(modelFile, data.in, inits, n.chain=3, n.adapt=1000)  
...
model{
  ...
  for (s in 1:spec){
    ...
    for (i in 1:Nmax){          		     # Nmax is higher than the max. number of observations for the most common species
