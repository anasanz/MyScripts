...
setwd("D:/PhD/Third chapter/Data/model/14.2.8_2")
cat("model{
...
n.burnin <- 30000
n.thin <- 2
model.file <- "model14.2.8_2.txt"

setwd("D:/PhD/Third chapter/Data/model/14.2.8_2")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="14.2.8_2.RData")

# With jagsUI 
out <- jags(data, inits, params, model.file = "model14.2.8_2.txt", n.chain,
...
load("D:/PhD/Third chapter/Data/model/14.2.8_2/JagsOutFOR14.2.8_2a.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/14.2.8_2/JagsOutFOR14.2.8_2b.RData")
outb <- out
#load("D:/PhD/Third chapter/Data/model/14.2.8/JagsOutFOR14.2.8c.RData")
#outc <- out
#class(outc)
...
for (c in 1:length(coeff)){
  ...
  setwd("D:/PhD/Third chapter/Data/Results_species/14.2/14.2.8_2")
  pdf(paste("14.2.8_2_", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
...
par(mfrow = c(3,1))
plot(out$mean$Tobsnewp ~ out$mean$Tobsp, main = "all species")
abline(v = 2, col = "red")
plot(out$mean$Tobsnewp[ ,26] ~ out$mean$Tobsp[ ,26], main = "sp26")
abline(v = 2, col = "red")
plot(out$mean$Tobsnewp[ ,27] ~ out$mean$Tobsp[ ,27], main = "sp27")
abline(v = 2, col = "red")

Tdf <- as.data.frame(out$mean$Tobsp)
Tdf_out <- apply(Tdf, 2, function(x) which(x > 2)) # MICAL, MECAL

# Identify which transect-years

# Mecal
sitesYears[Tdf_out$V26]
allyears[Tdf_out$V26]
all.sites[sitesYears[Tdf_out$V26]] # AL13 2015 y 2016

yLong.sp[Tdf_out$V26, 26] # 0 counts
yLong.sp[Tdf_out$V26,] 
...
green$area_green15[102]
green$area_green16[102]

# -> Maybe the model predicts high abundance in this transects??
yLong.sp[Tdf_out$V26, 26] # Counts in sites-year with high residuals
out$mean$lambda[Tdf_out$V26, 26] # Expected abundance in sites-years with high residuals
yLong.sp[ , 26]
out$mean$lambda[, 26]


# Mical
sitesYears[Tdf_out$V27]
allyears[Tdf_out$V27]
all.sites[sitesYears[Tdf_out$V27]]

# -> Maybe the model predicts high abundance in this transects??
yLong.sp[Tdf_out$V27, 27] # Counts in sites-year with high residuals
out$mean$lambda[Tdf_out$V27, 27] # Expected abundance in sites-years with high residuals
yLong.sp[ , 27]
out$mean$lambda[, 27]
...
# RANDON YEAR EFFECTS
out$mean$alam[c(26,27), ] # Dont show higher Random year effects in year 1
exp(out$mean$mu_l)
min(out$mean$alam)

setwd("D:/PhD/Third chapter/Data/Results_species/14.2/14.2.8_2")
pdf(paste("14.2.8_2_alam.pdf"))

par(mfrow = c(5,4),
    mar = c(2,2,2,0.5)) 

for (i in 1:nSpecies){
plot(out$mean$alam[i,] ~ yrs2, pch = 19, main = sp[i])}

dev.off()

# Calculate BP-Values for each year-transect combination
# IN MODEL:
# Sum residuals over sites and years to get sp-specific bp.obs.values
#T1obsp[s]<-sum(Tobsp[1:n.allSiteYear,s])
#T1obsnewp[s]<-sum(Tobsnewp[1:n.allSiteYear,s])
# Bp.Obs.sp[s] <-  T1obsp[s] > T1obsnewp[s]
# Select residuals of these two species
...
s <- 1
x <- list()
for(i in 1:dim(out$sims.list$Tobsp)[2]){
  print(
    x[i] <- sum(out$sims.list$Tobsp[,i,s] > out$sims.list$Tobsnewp[,i,s])/dim(out$sims.list$Tobsp)[1]
  )
}

df <- data.frame(year = m ,site = rep(all.sites,5), sp3 = NA, sp25 = NA, sp26 = NA)
sp1 <- c("sp3", "sp25", "sp26")
s <- c(3,25,26)
for (j in 1:length(sp1)){
  for(i in 1:dim(out$sims.list$Tobsp)[2]){
    print(
      df[i,colnames(df) %in% sp1[j]] <- sum(out$sims.list$Tobsp[,i,s[j]] > out$sims.list$Tobsnewp[,i,s[j]])/dim(out$sims.list$Tobsp)[1]
    )
  }}

df[which(df$mec < 0.2 | df$mec > 0.9), ]
i=50
hist(out$sims.list$Tobsp[,i,sp])
hist(out$sims.list$Tobsnewp[,i,sp],add=T, col=adjustcolor("red",alpha.f = 0.5))


mic <- data.frame(Tobsp = out$mean$Tobsp[ ,26],Tobsnewp = out$mean$Tobsnewp[ ,26])



#Save for Rahel
setwd("D:/PhD/Third chapter/Data/model/14.2.8")
save(all.sites, out, file="14.2.8_results.RData")
