...
# MODEL 14 in Community data (35 species). Calculate species specific BPvalues
# I haven't ran it (I pass to run the 14.2. DATA directly)

# sigma = exp(alpha(s) + observer(j,t) 
# lambda = exp(alpha(s,t) + sp.site (s,j) + b1*fallowSG(j,t) + b2(s)*fallowAES(j,t) + b3(s)*cropdiver(j,t)
...
data1 <- list(nyears = nyrs, max.sites = max.sites, nG=nG, siteYear.dclass = siteYear.dclass, int.w=int.w, strip.width = strip.width, 
              ...
              area1 = area_SG, area2 = area_AES, cdiv = crop_div,
...
cat("model{
    ...
    # SP-SPECIFIC BP.OBS
    for (s in 1:nSpecies){
    Tobsp.sp[s] <- Tobsp*indexSP[,s]
    Tobspnew.sp[s] <- Tobspnew*indexSP[,s]

    Bp.Obs.sp[s] <- sum(Tobspnew.sp[s]) > sum(Tobsp.sp[s])
    }
    ...
    file = "s_HNintegral_sigma[alpha(s)_obs(j,t)]_lambda[alpha(s,t)_sp.site(s,j)_covAreas(s,j,t)_covLands1(s,j,t)]_BPvaluesSP.txt")
...
params <- c( "mu_l", "sig_l", "sig_spsite",
             ...
             "bCropdiv", "mu_cd", "sig_cd",
             "sig.sig.ob",
             "mu_s", "sig_s", "Bp.N", "Bp.N.sp", "Bp.Obs", "Bp.Obs.sp")

# MCMC settings
nc <- 3 ; ni <- 200000 ; nb <- 30000 ; nt <- 10

# With jagsUI 
out <- jags(data1, inits, params, "s_HNintegral_sigma[alpha(s)_obs(j,t)]_lambda[alpha(s,t)_sp.site(s,j)_covAreas(s,j,t)_covLands1(s,j,t)]_BPvaluesSP.txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

setwd("D:/ANA/Results/chapter3")
save(out, file = "14_DATA_15sp.RData")
