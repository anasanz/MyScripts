...
# 15.1.0: Before adding ptori -> To check how was before adding ptori and if the results are the same
...
d <- read.csv("DataDS_ch3_15_19_READY_FIXED_beforeaddingptori.csv") #43 (-PTORI; +LAMIC, MIMIL, TUMER)
...
cat("model{
   ...
   # FOR BP.OBS
    ...
    file = "model15.1.0.txt")

# MODEL NAME:  "s_HR_beta(allsp)_sigma[alpha(s)_obs(j,t)]_lambda(rest)[alpha(s,t)_sp.site(s,j)_covAreas3(s,j,t)_covLands2(s,j,t)]_BPvaluesSP(resiN).txt"
...
# With jagsUI 
out <- jags(data1, inits, params, "s_HR_beta(allsp)_sigma[alpha(s)_obs(j,t)]_lambda(rest)[alpha(s,t)_sp.site(s,j)_covAreas3(s,j,t)_covLands2(s,j,t)]_BPvaluesSP(resiN).txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

setwd("D:/ANA/Results/chapter3")
save(out, file = "15.1_DATA_allsp_bp(resiN).RData") 
...
n.burnin <- 30000
n.thin <- 10
model.file <- "model15.1.0.txt"

setwd("D:/PhD/Third chapter/Data/model/15.1.0")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="15.1.0.RData")

# With jagsUI 
out <- jags(data, inits, params, model.file = "model15.1.0.txt", n.chain,
...
load("D:/PhD/Third chapter/Data/model/15.1.0/JagsOutFOR15.1.0a.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/15.1.0/JagsOutFOR15.1.0b.RData")
outb <- out
load("D:/PhD/Third chapter/Data/model/15.1.0/JagsOutFOR15.1.0c.RData")
outc <- out
class(outc)
...
source("D:/PhD/MyScripts/Ch. 2-3/Ch. 3/Data/PlotJagsParams.R")

param <- out$colnames.sims[grep("b.a2", out$colnames.sims)]
pdf("params_15.1.2.pdf")
...
for (c in 1:length(coeff)){
  ...
  setwd("D:/PhD/Third chapter/Data/Results_species/15.1/15.1.0_DATA_GOODsp_resiN_befptori")
  pdf(paste("15.1.0_DATA_GOODsp_resiN_befptori", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
