...
cat("model{
    ...
    file = "s_HR_beta(2groups)_sigma[alpha(s)_obs(j,t)]_lambda(rest)[alpha(s,t)_sp.site(s,j)_covAreas3(s,j,t)_covLands2(s,j,t)]_BPvaluesSP.txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_HR_beta(2groups)_sigma[alpha(s)_obs(j,t)]_lambda(rest)[alpha(s,t)_sp.site(s,j)_covAreas3(s,j,t)_covLands2(s,j,t)]_BPvaluesSP.txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

setwd("D:/ANA/Results/chapter3")
save(out, file = "15.2_DATA_allsp[bp].RData")
...
load("D:/PhD/Third chapter/Data/Results_model/15.2_DATA_allsp[bp].RData")
print(out)
...
for (c in 1:length(coeff)){
  ...
  pdf(paste("15.2_DATA_allsp[bp]_", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
