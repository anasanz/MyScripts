...
# Removed species with BAD p-VALUES and also PTORI (GOODsp)
#     # make model more efficient by putting *restrict.sp in the same line than lambda 
#     # that is the only difference between model 15.1.1 and model 15.1.2
...
# Restricted distribution of PTALC and CABRA (NOT PTORI!!!)
...
# Remove species with bad bp-values in AND PTORI
bad_bp <- c("GACRI", "GATHE", "PADOM", "STSSP","PTORI") 
...
cat("model{
    ...
    + b.a1[s]*area1[j] + b.a2[s]*area2[j] + b.a3[s]*area3[j] + bCropdiv[s]*cdiv[j] + bFieldsize[s]*fsiz[j] ) * restrict.sp[j,s]
...
# With jagsUI 
out <- jags(data, inits, params, "model15.1.2.txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

setwd("D:/PhD/Third chapter/Data/Results_model")
save(out, file = "15.1_DATA_GOODsp_bp(resiN).RData") 
...
n.burnin <- 30000
n.thin <- 10
model.file <- "model15.1.2.txt"

setwd("D:/PhD/Third chapter/Data/model")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="15.1.2.RData")
...
load("D:/PhD/Third chapter/Data/model/15.1.2/JagsOutFOR15.1.2a.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/15.1.2/JagsOutFOR15.1.2b.RData")
outb <- out
load("D:/PhD/Third chapter/Data/model/15.1.2/JagsOutFOR15.1.2c.RData")
outc <- out
class(outc)
...
for (c in 1:length(coeff)){
  ...
  setwd("D:/PhD/Third chapter/Data/Results_species/15.1/15.1.2_DATA_GOODsp_resiN")
  pdf(paste("15.1.2_DATA_GOODsp_resiN", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
