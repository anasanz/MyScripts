...
# Try joining GASSP to see if it improves the bpvalue and the skewness of AES
...
d <- d[-which(d$Species %in% c("PADOM", "STSSP")), ]
sort(unique(d$Species))
...
setwd("D:/PhD/Third chapter/Data/model/15.1.7")
cat("model{
...
n.burnin <- 30000
n.thin <- 10
model.file <- "model15.1.7.txt"

setwd("D:/PhD/Third chapter/Data/model/15.1.7")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="15.1.7.RData")

# With jagsUI 
out <- jags(data, inits, params, model.file = "model15.1.7.txt", n.chain,
...
load("D:/PhD/Third chapter/Data/model/15.1.7/JagsOutFOR15.1.7a.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/15.1.7/JagsOutFOR15.1.7b.RData")
outb <- out
load("D:/PhD/Third chapter/Data/model/15.1.7/JagsOutFOR15.1.7c.RData")
outc <- out
class(outc)
...
for (c in 1:length(coeff)){
  ...
  setwd("D:/PhD/Third chapter/Data/Results_species/15.1/15.1.7")
  pdf(paste("15.1.7_", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
