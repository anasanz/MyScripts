...
#     # that is the only difference between model 15.2.1 and model 15.2.2
...
bad_bp <- c("GACRI", "GATHE", "PADOM", "STSSP", "PTORI") # to remove all species with bad bp except MICAL and MECAL
...
setwd("D:/PhD/Third chapter/Data/model/15.2.2")
cat("model{
...
save(out, file = "15.2_DATA_GOODsp_bp(resiN).RData")
...
n.burnin <- 30000
n.thin <- 10
model.file <- "model15.2.2.txt"

setwd("D:/PhD/Third chapter/Data/model/15.2.2")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="15.2.2.RData")

# With jagsUI 
out <- jags(data, inits, params, model.file = "model15.1.2.txt", n.chain,
...
load("D:/PhD/Third chapter/Data/model/15.2.2/JagsOutFOR15.2.2a.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/15.2.2/JagsOutFOR15.2.2b.RData")
outb <- out
load("D:/PhD/Third chapter/Data/model/15.2.2/JagsOutFOR15.2.2c.RData")
outc <- out
class(outc)
...
for (c in 1:length(coeff)){
  ...
  setwd("D:/PhD/Third chapter/Data/Results_species/15.2/15.2.2_DATA_GOODsp_resiN")
  pdf(paste("15.2.2_DATA_GOODsp_resiN", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
