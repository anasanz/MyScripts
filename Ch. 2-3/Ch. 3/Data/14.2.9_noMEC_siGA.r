...
# - Calculating bp.obs values as Rahel said (NONE squared residuals)
# - TEST (very few iterations) 
# - No MICAL and si GASSP
...
d <- d[-which(d$Species %in% c("GACRI", "GATHE", "PADOM", "STSSP", "MICAL")), ]
sort(unique(d$Species))
...
setwd("D:/PhD/Third chapter/Data/model/14.2.9_noMEC_siGA")
cat("model{
    ...
    FT1[j,s] <- pow((N[j,s] - lambda.eff[j,s]),2)
    FT1new[j,s] <- pow((Nnew[j,s] - lambda.eff[j,s]),2)
    } 
    ...
    file = "model14.2.9_noMEC_siGA.txt")
...
n.burnin <- 30000
n.thin <- 2
model.file <- "model14.2.9_noMEC_siGA.txt"

setwd("D:/PhD/Third chapter/Data/model/14.2.9_noMEC_siGA")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="14.2.9_noMEC_siGA.RData")

# With jagsUI 
out <- jags(data, inits, params, model.file = "model14.2.9.txt", n.chain,
...
load("D:/PhD/Third chapter/Data/model/14.2.9_noMEC_siGA/JagsOutFOR14.2.9_noMEC_siGAa.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/14.2.9_noMEC_siGA/JagsOutFOR14.2.9_noMEC_siGAb.RData")
outb <- out
#load("D:/PhD/Third chapter/Data/model/14.2.9/JagsOutFOR14.2.9c.RData")
#outc <- out
#class(outc)
...
for (c in 1:length(coeff)){
  ...
  setwd("D:/PhD/Third chapter/Data/Results_species/14.2/14.2.9_noMEC_siGA")
  pdf(paste("14.2.9_noMEC_siGA_", names[c], ".pdf"))
  par(mfrow = c(5,4),
      mar = c(2,1,2,0.5)) 
...
plot(out$mean$Tobsnewp ~ out$mean$Tobsp)

Tdf <- as.data.frame(out$mean$Tobsp)
Tdf_out <- apply(Tdf, 2, function(x) which(x >10)) # MICAL, MECAL
###########################################################################################
