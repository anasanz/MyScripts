...
# REMOVE GASSP BECAUSE GATHE AND GACRI HAVE A LOT OF OBSERVATIONS: nooooot here

# JOIN GACRI y GATHE en GASSP

dat[which(dat$Species == "GASSP"), ]
dat$Species[which(dat$Species == "GATHE")] <- "GASSP" 
dat$Species[which(dat$Species == "GACRI")] <- "GASSP" 
...
setwd("D:/PhD/Third chapter/Data")
write.csv(dat,"DataDS_ch3_15_19_READY_FIXED_LAST_GASSP.csv") #Final data set ready for analyzing
