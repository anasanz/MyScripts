...
cat("model{
    ...
    for (s in 1:nSpecies){
      ...
      for(j in 1:n.allSiteYear){ 
        ...
        lambda[j,s] <- exp(log.lambda[sitesYears[j]]
    ...
    }",fill=TRUE, file = "s_sigma(integral)[alpha(s)_obs(j,t)]_lambda[alpha(j)_covArea(j,t)].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_sigma(integral)[alpha(s)_obs(j,t)]_lambda[alpha(j)_covArea(j,t)].txt", n.chain = nc,
...
traceplot(out, parameters = c( "mu.lam", "sig.lam", 
                               "ba1.lam", "ba2.lam",
                               "sig.sig.ob", 
                               "mu_s", "sig_s"))
summary <- as.data.frame(as.matrix(out$summary))
setwd("C:/Users/ana.sanz/Documents/PhD/Third chapter/Data/Model")
save(out, file = "9.1.1.2_S.RData")
