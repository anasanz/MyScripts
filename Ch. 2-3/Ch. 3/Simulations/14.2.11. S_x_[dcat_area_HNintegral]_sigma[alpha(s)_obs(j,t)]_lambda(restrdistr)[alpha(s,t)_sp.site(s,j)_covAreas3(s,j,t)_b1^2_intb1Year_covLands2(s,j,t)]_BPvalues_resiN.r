...
mu.a1.quad <- 0.8
sig.a1.quad <- 0.3
b.a1.quad <- rnorm(nSpecies, mu.a1.quad, sig.a1.quad)
...
lam <- exp(lam.alpha.spyear_data + 
             ...
             array(rep(b.a1.quad,each=max.sites*nyrs)*area1_sc*area1_sc, c(max.sites, nyrs, nSpecies)) +
...
setwd("D:/PhD/Third chapter/Data/model/14.2.6_SIM_2")
cat("model{
    ...
    b.a1.quad[s] ~ dnorm(mu_a1_quad, tau_a1_quad)
    ...
    mu_a1_quad ~ dnorm(0,0.01) # Hyperparameters for beta coefficient area1
    tau_a1_quad <- 1/(sig_a1_quad*sig_a1_quad)
    sig_a1_quad ~ dunif(0,500)
    ...
    + b.a1[s]*area1[j] + b.a1.quad[s]*area1[j]*area1[j] + b.a2[s]*area2[j] + b.a3[s]*area3[j] + bCropdiv[s]*cdiv[j] + bFieldsize[s]*fsiz[j] ) 
    ...
    file = "model14.2.11_SIM.txt")

# Inits
Nst <- (yLong.sp + 1)*restrict.sp
inits <- function(){list(mu_l = runif(1), sig_l = 0.2, sig_spsite = runif(1),
                         N=Nst,
                         mu_a1 = runif(1), sig_a1 = runif(1), mu_a1_quad = runif(1), sig_a1_quad = runif(1),
...
params <- c( "mu_l", "sig_l", "sig_spsite",
             "mu_a1", "sig_a1", "mu_a1_quad", "sig_a1_quad",
...
n.burnin <- 30000
n.thin <- 10
model.file <- "model14.2.11.txt"

setwd("D:/PhD/Third chapter/Data/model/14.2.11_SIM")
save(data, Nst, inits, params, n.chain, n.thin, n.iter, n.burnin, model.file, file="14.2.11_SIM.RData")

# With jagsUI 
out <- jags(data, inits, params, model.file = "model14.2.11_SIM.txt", n.chain,
            n.thin, n.iter, n.burnin, parallel = TRUE)


# To compare:
data_comp <- list(mu.a1 = mu.a1, sig.a1 = sig.a1, mu.a1.quad = mu.a1.quad, sig.a1.quad = sig.a1.quad,
...
load("D:/PhD/Third chapter/Data/model/14.2.11_SIM/JagsOutFOR14.2.11_SIMa.RData")
outa <- out
load("D:/PhD/Third chapter/Data/model/14.2.11_SIM/JagsOutFOR14.2.11_SIMb.RData")
outb <- out
load("D:/PhD/Third chapter/Data/model/14.2.11_SIM/JagsOutFOR14.2.11_SIMc.RData")
outc <- out
#class(outc)
...
mean <- unlist(out$mean)
q2.5 <- unlist(out$q2.5)
q97.5 <- unlist(out$q97.5)

for (i in 1:length(params)){
  print(paste(params[i], round(mean[i], 2), "[", round(q2.5[i], 2), "-", round(q97.5[i], 2), "]"))
}
