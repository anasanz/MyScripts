...
# RE: CHANGE RANDOM EFFECT IN LAMBDA, FROM SP-SITE TO ONLY SITE
...
cat("model{
    ...
    lambda[j,s] <- exp(alam[s,allyears[j]] + site[sitesYears[j]] + bzB.lam*zoneB[j]
    ...
    file = "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)_year(t)_covTemp(j)]_lambda[alpha(s,t)_site(j)_covZone(j)_covArea(j,t)].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)_year(t)_covTemp(j)]_lambda[alpha(s,t)_site(j)_covZone(j)_covArea(j,t)].txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

save(out, file = "RE_11.1_S.RData")
