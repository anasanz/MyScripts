...
# RE2: Try to make it converging by setting the first year of the random effects to 0 (in the model, nothing to do in simulation)
...
cat("model{
    ...
    file = "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)_year(t(t1=0))_covTemp(j)]_lambda[alpha(s,t(t1=0))_spsite(s,j)_covZone(j)_covArea(j,t)].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)_year(t(t1=0))_covTemp(j)]_lambda[alpha(s,t(t1=0))_spsite(s,j)_covZone(j)_covArea(j,t)].txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
save(out, file = "RE2_11.1_S.RData")
