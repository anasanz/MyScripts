...
#RE3: Use ZEPA as random effect instead of site
...
# RANDOM EFFECT IN SPECIES-ZEPA (Independent of year)
# There are 48 species, so there will be 48*7 = 336 values of zepa-sp combinations
zepa <- 1:7 
nzepa <- length(zepa)
sig.lam.spzepa <- 0.3

lam.spzepa <- rnorm(nSpecies*nzepa, 0, sig.lam.spzepa) # 1 value per species and zepa (instead of species and site)
lam.spzepa_order <- matrix(lam.spzepa, nrow = nzepa, ncol = nSpecies) # To index it later (because I need that all values come from the same distribution (or not really?), but that are different per species)

# Zepa covariate
zep.id <- matrix(sample(1:7, max.sites, replace = TRUE), nrow = max.sites, ncol = nyrs) # Matix with IDs

array_zep <- array(NA, c(max.sites, nyrs, nSpecies))


for (s in 1:nSpecies){
  zep <- matrix(lam.spzepa_order[zep.id,s],  nrow = max.sites, ncol = nyrs) # Matrix with value per species for simulating data
  array_zep[,,s] <- zep
}

# Check
lam.spzepa_order[ ,1] # Values for zepas species 1
unique(array_zep[,,1]) # Random effects created for species 1

lam.spzepa_order[ ,3] # Values for zepas species 3
unique(array_zep[,,3]) # Random effects created for species 3
...
data1 <- list(nyears = nyrs, max.sites = max.sites, nG=nG, siteYear.dclass = siteYear.dclass, int.w=int.w, strip.width = strip.width, midpt = midpt, 
              ...
              nSpecies = nSpecies, sp.dclass = sp.dclass, nyrs = nyrs, zep = zep, nzepa = nzepa)
...
cat("model{
    ...
    lambda[j,s] <- exp(alam[s,allyears[j]] + spzepa[s,zep[j]] + bzB.lam*zoneB[j]
    ...
    file = "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)_year(t)_covTemp(j)]_lambda[alpha(s,t)_spzepa(s,zepa)_covZone(j)_covArea(j,t)].txt")

# Inits
Nst <- yLong.sp + 1
inits <- function(){list(mu_l = runif(1), sig_l = 0.2, sig_spzepa = runif(1),
                         N=Nst,
...
# With jagsUI 
out <- jags(data1, inits, params, "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)_year(t)_covTemp(j)]_lambda[alpha(s,t)_spzepa(s,zepa)_covZone(j)_covArea(j,t)].txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)


save(out, file = "RE3_11.1_S.RData")
...
traceplot(out, parameters = c("mu_l", "sig_l", "sig_spzepa",
                              "bzB.lam", "ba1.lam", "ba2.lam",
                              "sig.sig.ob", "bTemp", "sig.sig.year",
                              "mu_s", "sig_s", "mu_b", "sig_b"))
