...
# RE:10.1 (CHANGE RANDOM EFFECT IN LAMBDA, FROM SP-SITE TO ONLY SITE) 
# Modify it as if I would add only the variables from one hypothesis (Good scenario for biodiversity):
  # lambda = alpha(s,t) + site(j) + b1*FallowSG + b2*CropDiv + b3*FieldSize
# Just remove 2 area covariates
...
# RANDOM EFFECT IN SITE (NOT species, so if a site is better, its better for all species)
sig.lam.site <- 0.3
lam.site <- rnorm(max.sites, 0, sig.lam.site) 
ar <- array(rep(lam.site, each = nyrs), c(nyrs, max.sites, nSpecies)) # I need to make it in wide format and transpose it to long
ar1 <- list() 
for (i in 1:nSpecies){
...
lam.site_data <- array(as.numeric(unlist(ar1)), c(max.sites, nyrs, nSpecies))
...
data1 <- list(nyears = nyrs, max.sites = max.sites, nG=nG, siteYear.dclass = siteYear.dclass, int.w=int.w, strip.width = strip.width, midpt = midpt, 
              ...
              area1 = area1, cdiv = cdiv, fsiz = fsiz, ob = ob, nobs = nobs, db = dist.breaks,
...
cat("model{
    ...
    for (i in 1:max.sites){           # Random effect for lambda (dif abundance per site)
    ...
    tau_site <- 1/(sig_site*sig_site) # Hyperparameter for site random effect in lambda
    sig_site ~ dunif(0,500)
    ...
    lambda[j,s] <- exp(alam[s,allyears[j]] + site[sitesYears[j]]
    + ba1.lam*area1[j] 
    ...
    file = "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)]_lambda[alpha(s,t)_site(j)_covArea1(j,t)_covLands(j,t)].txt")

# Inits
Nst <- yLong.sp + 1
inits <- function(){list(mu_l = runif(1), sig_l = 0.2, sig_site = runif(1),
                         N=Nst,
                         ba1.lam = runif(1),
...
# Params
params <- c("mu_l", "sig_l", "sig_site",
            "ba1.lam", "b.cdiv.lam", "b.fsiz.lam",
            "sig.sig.ob",
            "mu_s", "sig_s", "mu_b", "sig_b")

# MCMC settings
nc <- 3 ; ni <- 70000 ; nb <- 10000 ; nt <- 10

# With jagsUI 
out <- jags(data1, inits, params, "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)]_lambda[alpha(s,t)_site(j)_covArea1(j,t)_covLands(j,t)].txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

save(out, file = "RE_10.1.3_S.RData")
...
traceplot(out, parameters = c("mu_l", "sig_l", "sig_site",
                              "ba1.lam", "b.cdiv.lam", "b.fsiz.lam",
                              "sig.sig.ob",
                              "mu_s", "sig_s", "mu_b", "sig_b"))
