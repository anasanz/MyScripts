...
### sp-ZEPA effect independent of year
### 2 areas variables
...
data1 <- list(nyears = nyrs, max.sites = max.sites, nG=nG, siteYear.dclass = siteYear.dclass, int.w=int.w, strip.width = strip.width, midpt = midpt, 
              ...
              area1 = area1, area2 = area2, ob = ob, nobs = nobs, db = dist.breaks, zep = zep, nzepa = nzepa,
...
cat("model{
    ...
    file = "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)]_lambda[alpha(s,t)_spzepa(s,zepa)_covArea(j,t)].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_HRdetect_beta(s)_sigma[alpha(s)_obs(j,t)]_lambda[alpha(s,t)_spzepa(s,zepa)_covArea(j,t)].txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

save(out, file = "RE3_10.1_S.RData")
