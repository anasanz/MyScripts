...
# 1.2. Try random intercept in site to see if it converges
...
# N[jt] ~ Pois(lambda[jt])
# log(lambda[jt]) <- site.alpha.ran[j] + year[t] + beta*yr[t]
...
data1 <- list(nyears = nyrs, nsites = max.sites, nG=nG, int.w=int.w, strip.width = strip.width, midpt = midpt, db = dist.breaks,
              ...
              zoneB = zB, ob = ob.id, nobs = nobs, year1 = year_number, site = site)
...
cat("model{
    ...
    for(j in 1:nsites){ 
      ...
      lambda[j,t] <- exp(log.lambda[site[j]] + year[t] + bYear.lam*year1[t])
    ...
    }",fill=TRUE, file = "s_sigma(integral)[obs(o,j,t)_covZone(j)]_lambda(PoisGam)[alpha.site.random(j)_year(t)_beta.year(j)].txt")


# Inits
Nst <- y.sum + 1
inits <- function(){list(mu.sig = runif(1, log(30), log(50)), sig.sig = runif(1), bzB.sig = runif(1),
                         mu.lam = runif(1), sig.lam = 0.2, year = rnorm(nyrs), bYear.lam = runif(1),
                         N = Nst)} 

# Params
params <- c( "mu.sig", "sig.sig", "bzB.sig",
             "mu.lam", "sig.lam", "year", "bYear.lam",
...
# With jagsUI 
out2 <- jags(data1, inits, params, "s_sigma(integral)[obs(o,j,t)_covZone(j)]_lambda(PoisGam)[alpha.site.random(j)_year(t)_beta.year(j)].txt", n.chain = nc,
