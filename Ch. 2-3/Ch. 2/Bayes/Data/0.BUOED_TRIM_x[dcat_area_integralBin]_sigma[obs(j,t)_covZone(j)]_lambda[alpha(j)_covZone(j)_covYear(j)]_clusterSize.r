...
mec <- d[which(d$Species == "BUOED"), which(colnames(d) %in% c("Year", "Banda", "transectID", "T_Y", "Species", "Observer", "Count"))] # Select species BUOED and all years
...
write.csv(summary, "8.TRIM_Buoed.csv")

###################################################################

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/TRIM")
summary <- read.csv("8.TRIM_Buoed.csv")
...
title("STONE CURLEW", line = -1, cex = 2, outer = TRUE)
...
plot(pred ~ yrs2, ylim=c(0,1), type="l", main = "Stone curlew", xlab = "Year", ylab = "Abundance")
