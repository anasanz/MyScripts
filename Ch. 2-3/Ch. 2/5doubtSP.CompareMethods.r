...
for (xxx in 1:length(s_good)){
  ...
  save(out, file = paste("5HDS_doubt_",s_good[xxx],".RData", sep = ""))
  
  
  # ---- Results ----
  
  setwd("S:/PhD/Second chapter/Data/Results/TRIM/5allcov")
  load(paste("5HDS_doubt_",s_good[xxx],".RData", sep = ""))
  ...
  pdf(paste(s_good[xxx],"_TrimComp5_doubt.pdf", sep = ""), height = 5, width = 9)
  ...
  write.csv(coef_dev, file = paste("coef_dev_doubt",s_good[xxx],".csv", sep = ""))
