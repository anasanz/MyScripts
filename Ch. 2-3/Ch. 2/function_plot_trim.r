...
#'
#' @export
plot.trim.overall_yo <- function(x, yrange = c(0, 100), imputed=TRUE, ...) {
  #browser()
  X <- x
  ...
  plot(xrange, yrange, type='n', xlab=" ", ylab= " ", las=1, main=title, cex.lab = 1.2, cex.axis = 1.2, ...)
  
  #axis(side = 2, cex = 1.2)
  #axis(side = 1, cex = 1.2)
  
  mtext("Number of individuals", side = 2, line = 3, cex = 1.2)
  mtext("Year", side = 1, line = 3, cex = 1.2)
  
  polygon(conf.band, col= adjustcolor("red",alpha.f = 0.2), lty=0)
