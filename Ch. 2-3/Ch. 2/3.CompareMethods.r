...
for (xxx in 1:length(s_good)){
  ...
  significance_est_dev <- ifelse(sig_dev$deviations$p < 0.05, 
                             paste(est,"*"), 
                             est)
  
  col_est <- ifelse(est > 0, "blue", "red")
  
  text(2017.5,1.5, significance_est, col = col_est) # Significance for the slope in the right and 
  text(2011,1.5, significance_est_dev, col = col_est) # significance for deviations from the trend in the left
