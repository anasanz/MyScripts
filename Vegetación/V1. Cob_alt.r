...
setwd("C:/Users/ana.sanz/Documents/Datos/Datos barbechos arrendados/Consulta")
...
#Eliminar variables 6b,7b,8b,9b de las variables cob_altura asignandoselo a la categor?a 6,7,8,9
...
#Los Na aparecen porque el ?ndice es 0 al haber s?lo una clase del 100% (Luego la herogeneidad es muy 
#baja, los Na podrian ser considerados 0). El ?ndice sale 1 y cuando lo estandarizas es 0/0 = NA
...
which(wid$lev_ind > 1) #12 observaciones ... merece la pena a?adir esta variable?
...
ab<-read.csv("Birddata_abundance_FINAL.csv",sep = ";",
             header=TRUE,fill = TRUE)
ab<-ab[-which(ab$Contatge == 99),]
wid<-spread(ab,especieObjectiu,Contatge) 
wid$CF_A<-paste(wid$Codi_Finca,wid$Any,sep="_")
...
wid$lev_ind<-wide$ind
wid<-wid[ ,-c(6:11,16,18,20,22,24,26,28:35)]
wid$lev_ind[wid$lev_ind == -Inf]<-0
wid$lev_ind[wid$lev_ind == Inf]<-0
wid$lev_ind[is.na(wid$lev_ind)]<-0
setwd("C:/Users/ana.sanz/Documents/Datos/Datos barbechos arrendados/Variables")
...
# Hay filas que NO SUMAN 100, por lo que el levin index sale mayor que uno
which(wid$lev_ind > 1) #18 observaciones ... merece la pena añadir esta variable?
wid$lev_ind[which(wid$lev_ind > 1)] <- NA

#write.csv(wid,file = "Fallow_heterogeneity_FINAL.csv") 
