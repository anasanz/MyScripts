...
# Zone variable. AREAS COVARIATES REMOVED BECAUSE IT DIDNT CONVERGE. NOW IT CONVERGES AND
...
cat("model{
    ...
    }",fill=TRUE, file = "s_sigma_lambda[alpha(j)_covZones(j)_covAreas(jt)_REMOVE_AREA].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_sigma_lambda[alpha(j)_covZones(j)_covAreas(jt)_REMOVE_AREA].txt", n.chain = nc,
