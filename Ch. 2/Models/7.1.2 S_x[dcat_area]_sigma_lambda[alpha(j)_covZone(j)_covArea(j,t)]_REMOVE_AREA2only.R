...
#+ ba2.lam*area2[j]
    ...
    }",fill=TRUE, file = "s_sigma_lambda[alpha(j)_covZones(j)_covAreas(jt)_REMOVE_AREA2].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_sigma_lambda[alpha(j)_covZones(j)_covAreas(jt)_REMOVE_AREA2].txt", n.chain = nc,
