...
nbins <- length(unique(mec$Banda)) # Number of bins
...
nau <- 10000 # Augment observed data with 300 observations more (we dont know if are detections or not)
y <- c(rep(1, n), rep(0, nau)) # Vector describing observed (1) and augmented (0) individuals
xbin1 <- c(mec$Banda, rep(NA, nau)) # Vector with distances observed (x, known) and augmented (NA)
...
cat("
    ...
    D <- N / 0.1
    }",fill=TRUE, file = "model_different_binsize_D_Mecal.txt")
...
out <- jags(win.data, inits, params, "model_different_binsize_D_Mecal.txt", n.chains = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
traceplot(out, param = c("N", "sigma", "D"))
print(out, 2)

setwd("C:/OneDrive/PhD/Second chapter/Data/Model/Plots")
pdf(file = "3.1S_x[dcat_area]_sigma_D_Mecal.pdf")
