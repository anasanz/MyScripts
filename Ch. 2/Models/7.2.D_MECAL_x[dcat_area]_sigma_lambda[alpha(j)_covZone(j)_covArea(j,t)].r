...
results <- summary[which(row.names(summary) %in% c("Ntotal[1]", "Ntotal[2]", "Ntotal[3]", "Ntotal[4]", "Ntotal[5]", "Ntotal[6]",
...
axis(2)
points(results[1:8,1],pch = 19)
x <- seq_along(results[1:8,1])
arrows(x,results$`2.5%`[1:8],x, results$`97.5%`[1:8], code=3, angle=90, length=0.04) #???MAL
