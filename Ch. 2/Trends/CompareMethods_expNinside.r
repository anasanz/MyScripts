...
cat("model{
    ...
    # Calculate expected abundance per year inside model

    lam.star.tot[1] <- lamtotal_clus[1] # Expected abundance in year 1
    for (i in 2:nyears){
      lam.star.tot[i] <- lam.star.tot[i-1] * # Here I add the starting population size as a baseline for the trend 
        exp(bYear.lam)}
    
    
    }",fill=TRUE, file = "s_sigma(integral)[obs(o,j,t)_covZone(j)]_lambda(PoisGam)[alpha(j)_covZone(j)_year(j)]_clustersize_expN.txt")
...
params <- c("Ntotal", "Ntotal_clus", #"N", "sigma", "lambda", I remove it so that it doesnt save the lambdas and takes shorter. It still calculates them
            ...
            "lamtotal", "lamtotal.P", "lam.star.tot"
)
...
# With jagsUI 
out2 <- jags(data1, inits, params, "s_sigma(integral)[obs(o,j,t)_covZone(j)]_lambda(PoisGam)[alpha(j)_covZone(j)_year(j)]_clustersize_expN.txt", n.chain = nc,
             n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
print(out2)
#s2 <- as.data.frame(out2$summary)
setwd("C:/Users/ana.sanz/Documents/PhD/Second chapter/Data/Results/TRIM")
save(out2, file = "8.TRIM_Terax_expN.RData")
...
#1. With yearly expected N calculated inside model

setwd("C:/Users/ana.sanz/Documents/PhD/Second chapter/Data/Results/TRIM")
load("8.TRIM_Terax_expN.RData")
...
plot(-15, xlim=c(0,8), ylim=c(0,200), main = "exp_in", xlab = "Year", ylab = "Abundance")
polygon( x = c(yrs2, rev(yrs2)),
         y = c(results[14:22,3], rev(results[14:22,7])), 
         col = adjustcolor(c("grey"),alpha.f = 0.6),
         border = NA)
points(results[14:22,1] ~ yrs2, type="l", col = "red")
...
# 2. Calculating expected abundance outside from parameters
...
plot(-15, xlim=c(0,8), ylim=c(0,200), main = "exp_out", xlab = "Year", ylab = "Abundance")
...
############################################################################################################
#####                       CONCLUSIÓN: INSIDE OR OUTSIDE, SAME RESULTS                             #######
############################################################################################################
