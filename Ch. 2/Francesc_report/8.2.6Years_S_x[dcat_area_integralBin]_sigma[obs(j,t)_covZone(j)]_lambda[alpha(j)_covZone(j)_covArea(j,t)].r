...
# Model with 6 years to then analyze it from 2013 - 2018 for report francesc
# It converges and retrieves the estimates, but when including the 2 area variables it is 
#not very accurate for 2 pop estimates (like in the one with 9 years), so Im a bit suspicious

# WITH ONE AREA VARIABLE IT WORKS PERFECTLY

# lambda site specific(Zone variable and ONLY ONE AREA VARIABLE, BECAUSE i DONT HAVE AES 2018)
...
nSites <- seq(50,90, by = 8)				# number of line transect surveys (DIFFERENT BY YEAR)
...
#Covariates
a2 <- abs(rnorm(max.sites*nyrs, 5, 2.5)) # Although it makes sense to make them positive, it wouldnt matter (you put them on the exp)
...
a2.m <- matrix(area2_sc, nrow = max.sites, ncol = nyrs, byrow = F)# I need to make it from the same matrix 
                                                                  # from which I created lambda, to make it fit!
...
data1 <- list(nyears = nyrs, max.sites = max.sites, nG=nG, siteYear.dclass = siteYear.dclass, int.w=int.w, strip.width = strip.width, 
              ...
              area2 = area2, zoneB = zoneB, ob = ob, nobs = nobs, db = dist.breaks)
...
# To compare:
data_comp <- list(N.tot = N.tot, b.a2 = b.a2, b.lam.zoneB = b.lam.zoneB,
