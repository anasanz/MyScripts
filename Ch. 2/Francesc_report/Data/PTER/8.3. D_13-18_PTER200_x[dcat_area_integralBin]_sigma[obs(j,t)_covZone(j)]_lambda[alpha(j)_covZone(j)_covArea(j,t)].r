...
# Run model 8.2 in SAND dataset. BUFFER 200 SG
...
# 2. Join the observations of SAND (for example) with all transects so that they remain with NA if the
...
mec <- d[which(d$Species == "SAND"), which(colnames(d) %in% c("Year", "Banda", "transectID", "T_Y", "Species", "Observer"))] # Select species MECAL and all years
...
print(out) # DOESNT CONVERGE

summary <- as.data.frame(as.matrix(out$summary))

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
write.csv(summary, "8.2.Pter200_13-18.csv") # DOESNT CONVERGE

###################################################################

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
summary <- read.csv("8.2.Pter200_13-18.csv") # DOESNT CONVERGE
...
plot(-100,ylim = c(0,120), xlim=c(0,8),
     pch = 21, ylab = "N", xlab = " ", axes = FALSE, main = "Pter")
...
plot(pred ~ area_SGpred, ylim=c(0,4), type="l", main = "SISÓ")
