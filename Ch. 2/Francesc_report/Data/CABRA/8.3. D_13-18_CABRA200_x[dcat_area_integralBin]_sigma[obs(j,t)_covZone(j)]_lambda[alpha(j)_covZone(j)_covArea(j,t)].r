...
# Run model 8.2 in CABRA dataset. BUFFER 200 SG
...
# 2. Join the observations of CABRA (for example) with all transects so that they remain with NA if the
...
mec <- d[which(d$Species == "CABRA"), which(colnames(d) %in% c("Year", "Banda", "transectID", "T_Y", "Species", "Observer"))] # Select species MECAL and all years
...
print(out) # DOESN'T CONVERGE

summary <- as.data.frame(as.matrix(out$summary))

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
write.csv(summary, "8.2.Cabra200_13-18.csv") # DOESN'T CONVERGE

###################################################################

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18") # DOESN'T CONVERGE
summary <- read.csv("8.2.Cabra200_13-18.csv")
...
plot(-100,ylim = c(0,120), xlim=c(0,8),
     pch = 21, ylab = "N", xlab = " ", axes = FALSE, main = "CABRA")
axis(1, at = c(1,2,3,4,5,6), labels = yrs)
axis(2)
points(results500[1:6,2],pch = 19) # Plot results
points(count.year,pch = 19, col = "red") # Plot counts
x <- seq_along(results500[1:6,2])
low_CI <- as.numeric(results500$X2.5.[1:6])
up_CI <- as.numeric(results500$X97.5.[1:6])
arrows(x, low_CI,x, up_CI, code=3, angle=90, length=0.04) 


# To plot the relation with the co-variates 
results200_2 <- summary[7:888, ]
...
plot(pred ~ area_SGpred, ylim=c(0,4), type="l", main = "SISÃ“")
