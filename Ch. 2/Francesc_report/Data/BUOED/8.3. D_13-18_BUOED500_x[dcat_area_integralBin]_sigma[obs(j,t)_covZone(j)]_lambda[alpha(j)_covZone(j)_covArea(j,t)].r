...
# Run model 8.2 in BUOED dataset. BUFFER 200 SG
...
write.csv(summary, "8.2.Buoed500_13-18.csv")

###################################################################

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
summary <- read.csv("8.2.Buoed500_13-18.csv")
...
pdf("Buoed_500_HA_SG_1318.pdf")

area_SGpred <- seq(min(area_SG_HA), 40,length.out = 200) # Create a sequence of values, from minimum to maximun of the covariate to plot the prediction
                                                        # i TAKE 40 BECAUSE THE GRAPH IS A BIT WEIRD OTHERWISE
...
plot(pred ~ area_SGpred, ylim=c(0,4), type="l", main = "TORLIT", xlab = "Guarets gestionats (HA)", ylab = "Abundància")
