...
# Run model 8.2 in TERAX dataset. BUFFER 200 SG
...
data1 <- list(nyears = nyrs, max.sites = max.sites, nG = nG, siteYear.dclass = siteYear.dclass, int.w=int.w, strip.width = strip.width, 
              ...
              area2 = area_SG, ob = ob, nobs = nobs, db = dist.breaks)
...
cat("model{
    ...
    }",fill=TRUE, file = "s_sigma(integral)[obs(o,j,t)]_lambda[alpha(j)_covArea2(j,t)].txt")
...
# With jagsUI 
out <- jags(data1, inits, params, "s_sigma(integral)[obs(o,j,t)]_lambda[alpha(j)_covArea2(j,t)].txt", n.chain = nc,
...
write.csv(summary, "8.2.Terax200_ORIENTALES_13-18.csv")

###################################################################

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
summary <- read.csv("8.2.Terax200_13-18.csv")
