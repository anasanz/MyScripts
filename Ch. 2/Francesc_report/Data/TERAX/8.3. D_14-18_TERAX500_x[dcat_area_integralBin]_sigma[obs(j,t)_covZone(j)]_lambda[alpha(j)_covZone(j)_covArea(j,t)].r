...
which(summary$Rhat > 1.1) # Only one value is 1.11, so it converged

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
write.csv(summary, "8.2.Terax500_14-18.csv")

###################################################################

setwd("C:/Users/ana.sanz/OneDrive/PhD/Second chapter/Data/Results/8.2.Francesc13-18")
summary <- read.csv("8.2.Terax500_14-18.csv")

results500 <- summary[which(summary$X %in% c("Ntotal[1]", "Ntotal[2]", "Ntotal[3]", "Ntotal[4]", "Ntotal[5]", "mu.lam", "sig.lam", "bzB.lam", "ba1.lam", "ba2.lam")), ]


# SG POSITIVE EFFECT, ORIENTALES POSITIVE EFFECT

# Plot the trend of the population
plot(-100,ylim = c(0,120), xlim=c(0,8),
     pch = 21, ylab = "N", xlab = " ", axes = FALSE, main = "TERAX")
axis(1, at = c(1,2,3,4,5), labels = yrs)
axis(2)
points(results500[1:5,2],pch = 19) # Plot results
points(count.year,pch = 19, col = "red") # Plot counts
x <- seq_along(results500[1:5,2])
low_CI <- as.numeric(results500$X2.5.[1:5])
up_CI <- as.numeric(results500$X97.5.[1:5])
arrows(x, low_CI,x, up_CI, code=3, angle=90, length=0.04) 


# To plot the relation with the co-variates 
results500_2 <- summary[6:730, ]
...
plot(pred ~ area_SGpred, ylim=c(0,4), type="l", main = "TERAX buffer.500")
