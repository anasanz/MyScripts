...
for (i in 1:nsim) {
  ...
  y<-sapply(dat$detList, rowSums) # All the counts
  ...
  for (t in 1:nyrs){
    for (j in 1:npoints){
      ...
      ssi<-rep(j, y[j,t]) # site index: repeat the site as many times as counts in that site
                          # vector of sites through years (disregarding distance class)
      dc<-NULL
      for (k in 1:nG){
        if(dat$detList[[t]][j,k]==0) next # detList is the list of the dist clases per year (6 elements in the list)
        dd<-rep(k, dat$detList[[t]][j,k]) # Repeat the dclass as many times as counts in the year t of the list
        dc<-c(dc, dd)
      }
...
dsim.fun<-function(dmax=300, lamnew=lamnew,sigma=sigma,surv=surv, rec=rec, npoints=npoints){
  ...
  NcList<-list(Nc,Nc,Nc,Nc,Nc,Nc) #? Data on observations for each year
