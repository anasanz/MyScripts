...
# PERIODO 1 (PRE)
  ...
  newdata_asp <- as.data.frame(lapply(lapply(datos_period_Pre[, c(25:26)], mean), rep, 250)) # Mean of caminos y slope
  ...
  inv.logit <- function(inValues) { # To backtransform (from linear response to probabilities from 0-1)
    1.0 / (1.0 + exp(-inValues))}
      ...
      points(datos_period_Pre$STATUS ~ datos_period_Pre$carreteras.st) # Raw data
  ...
  ##########################################################################################################################
      ...
      newdata_asp <- as.data.frame(lapply(lapply(datos_period_PreRep[, c(25:26)], mean), rep, 250)) # Mean of caminos y slope
