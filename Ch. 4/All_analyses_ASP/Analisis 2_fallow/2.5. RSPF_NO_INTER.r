...
for (p in 1:length(periods)){
  ...
  setwd("D:/PhD/Fourth chapter/Results/RSF_Ana/Analisis 2/result_prov_rspf_no_inter")
  write.csv(results, file = paste("St1.FResults_", periods[p],"_", "dm_A1.csv", sep = ""))
  
}

## ---- St 2: used ~ no_manag + si_manag + slope ----
...
## ---- St 3: used ~ no_manag + si_manag + linear.st + slope  ----
...
for (p in 1:length(periods)){
  ...
  #for (i in 1:n.loop){
  RSF <- rspf(STATUS ~ as.factor(no_manag) + as.factor(si_manag) + linear.st + Slope , data = datos.rspf, m = match.use.avai, 
              B = n.B)
