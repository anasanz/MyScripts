...
CRS.used <- bar17@proj4string # Only to have the reference crs


# Random locations
temp <- setwd("D:/PhD/Fourth chapter/Results/RSF_Ana/Analisis 2/random_loc/Revision")
...
##REV: Set the used dataset in the same order and columns as it was before, so the script matches
used <- used[,-c(2,4,8,24:27)]

# Year2 is the dun-year and the "academic" year of our study so it should be the year cov
colnames(used)[17] <- "year"
used <- used[ ,c(1:2,17,3:16,18:20)]
...
## REV: Here because the random and used locations already are assign to the dun year, 
## it will be extracted correctly
