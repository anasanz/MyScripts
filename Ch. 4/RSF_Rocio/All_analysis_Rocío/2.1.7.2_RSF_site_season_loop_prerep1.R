...
#                   PERIODO PREREP (DESDE 20 DE MARZO - 6 JUNIO)
#                              ROCIO TARJUELO
#-----------------------------------------------------------------------------------------
...
# Seleccionar sólo los datos para el periodo prerep
datos.use.rspf <- subset(datos, periodo%in%"PreRep" & STATUS == 1)
datos.avai.rspf <- subset(datos, periodo%in%"PreRep" & STATUS == 0)
...
names(RSF1.pval.mean) <- rownames(RSF$coefficients)

RSF1.SE.mean <- round(apply(RSF1.SE, 1, mean), 3)
names(RSF1.SE.mean) <- rownames(RSF$coefficients)
