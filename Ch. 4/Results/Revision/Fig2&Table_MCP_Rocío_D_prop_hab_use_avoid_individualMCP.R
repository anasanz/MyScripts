...
for (i in 1:length(pre_id)){
  all_data[rownames(all_data) %in% pre_id[i], c(1:2)] <-  pre[pre$Bird.ID %in% pre_id[i], c(2,3)]
}
...
setwd("D:/PhD/Fourth chapter/Results/Figures_Tables/Revision")
...
## REV: En la revisión esto no cambia porque siempre seleccionan barbecho
# With % of each use in each period
