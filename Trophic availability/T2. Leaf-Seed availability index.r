...
fl<-read.csv("Cover_traits.csv",sep = ",",header=TRUE,fill = TRUE,na.strings="")
fl<-fl[which(fl$Any != "2014"),] 
fl$X<-paste(fl$Codi_Finca, fl$Any, sep = "_")

#Selected species that I will use to build the index 
#(described in T0: Más frecuentes, y de las menos, aquellas que tienen más de un 20% de cobertura en alguna en alguna finca)


#2. HEIGHT
#Dato campo:
ab<-read.csv("Birddata_abundance.csv",sep = ";",header=TRUE,fill = TRUE) #birddata_abundance es el archivo mas reciente de abundancia (9/10)
ab<-ab[which(ab$EspecieObj %in% c("CABRA")),c(1,2,3,5,6,7,11,12,13)]
...
alt<-data.frame(matrix(nrow = 489, ncol = 76))
colnames(alt)<-dat$V1
...
alt$X<-un$X
alt<-alt[,c(77,1:76)] #HEIGHT OF ALL PLANTS
