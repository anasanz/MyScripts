...
tor <- read.csv("tortola_ds_ready_reobs.csv", sep = ",") # I use this one because has all observers and we are not using observer in this model
tor[,1] <- "STTUR"
###################################################################
##                       HDS ANALYSIS                           ###
###################################################################
...
# Add forest for fields with counts > 0
for (i in 1:nrow(tor)){
